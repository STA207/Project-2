...
Due to unbalanced design, the factor effect component sum of squares are no longer orthogonal. Therefore, we would use the general linear F-test for the testing parts. The basic idea is to compare SSE under the full model with SSE under the reduced model, and we want to test whether specific components could be drop out of the full model. The details are shown as below:
...
###### 2.2.3.1.2 Pairwise Class Size Effects
